...
------------------------------
  ...
  q       = 1.0  n/cm^3/s    (layers 1-4, fixed)

Varying parameter
-----------------
  q_5 ∈ [0, 3]  n/cm^3/s    (layer 5 only, sampled via LHS)
...
# Degenerate bounds [v, v] → fixed; only q for layer 5 is open
D_bounds       = [[D_FIXED,     D_FIXED]]     * N_LAYERS
sigma_a_bounds = [[SIGMA_FIXED, SIGMA_FIXED]] * N_LAYERS
q_bounds       = [[Q_FIXED, Q_FIXED]] * (N_LAYERS - 1) + [Q5_RANGE]

sampler = DiffusionParamSampler(N_LAYERS, D_bounds, sigma_a_bounds, q_bounds)
X = sampler.sample(M_SAMPLES, random_state=SEED)

# q_5 values actually sampled (column index 2*N_LAYERS + 4, i.e. last param)
q5_values = X[:, -1]
print(f"q_5 sample range: [{q5_values.min():.3f}, {q5_values.max():.3f}]")
...
cbar.set_label("$q_5$  [n cm$^{-3}$ s$^{-1}$]", fontsize=11)

ax.set_xlabel("x  [cm]", fontsize=12)
ax.set_ylabel("φ(x)  [a.u.]", fontsize=12)
ax.set_title(
    f"5-layer slab – varying $q_5 \\in {Q5_RANGE}$\n"
