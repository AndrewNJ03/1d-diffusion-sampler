"""
5-layer diffusion study: varying cross section in layer 5 only.

Geometry
--------
  Domain       : x ∈ [0, 10] cm, 5 equal layers of width 2 cm each
  Layer bounds : [0, 2, 4, 6, 8, 10]

Fixed parameters (layers 1-5)
------------------------------
  D       = 1.0  cm          (all layers)
  Sigma_a = 0.5  cm^-1       (layers 1-4, fixed)
  q       = 1.0  n/cm^3/s    (all layers)

Varying parameter
-----------------
  Sigma_a_5 ∈ [0.0, 1.5]  cm^-1    (layer 5 only, sampled via LHS)

BCs    : zero-flux Dirichlet on both ends (φ(0) = φ(10) = 0)
Mesh   : N = 200 cells
Plot   : solutions restricted to x ∈ [1, 5]
"""

import numpy as np
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt

from second_order_solver import solve_diffusion
from lhs_generation import DiffusionParamSampler
from masking_function import mask_solution

# ------------------------------------------------------------------ #
# Geometry                                                            #
# ------------------------------------------------------------------ #
L            = 10.0
N_LAYERS     = 5
N_CELLS      = 200
layer_bounds = np.linspace(0.0, L, N_LAYERS + 1)   # [0,2,4,6,8,10]

PLOT_MIN = 1.0
PLOT_MAX = 5.0

# ------------------------------------------------------------------ #
# Parameter setup                                                      #
# ------------------------------------------------------------------ #
D_FIXED     = 1.0
SIGMA_FIXED = 1.0
XS5_RANGE   = [0.5, 1.5]   # only layer 5 varies
Q_FIXED     = 1.0

M_SAMPLES = 40
SEED      = 0

# Degenerate bounds [v, v] → fixed; only sigma_a for layer 5 is open
D_bounds       = [[D_FIXED,     D_FIXED]]     * N_LAYERS
sigma_a_bounds = [[SIGMA_FIXED, SIGMA_FIXED]] * (N_LAYERS - 1) + [XS5_RANGE]
q_bounds       = [[Q_FIXED, Q_FIXED]] * N_LAYERS

sampler = DiffusionParamSampler(N_LAYERS, D_bounds, sigma_a_bounds, q_bounds)
X = sampler.sample(M_SAMPLES, random_state=SEED)

# sigma_a_5 values actually sampled (column index N_LAYERS + N_LAYERS - 1 = 9)
sigma_a5_values = X[:, N_LAYERS * 2 - 1]
print(f"Sigma_a_5 sample range: [{sigma_a5_values.min():.3f}, {sigma_a5_values.max():.3f}]")

# ------------------------------------------------------------------ #
# Solve for each sample                                               #
# ------------------------------------------------------------------ #
fig, ax = plt.subplots(figsize=(8, 5))

cmap   = plt.get_cmap('plasma')
norm   = plt.Normalize(vmin=XS5_RANGE[0], vmax=XS5_RANGE[1])

phi_at_x1 = []
phi_at_x5 = []

for i, g in enumerate(X):
    D_i, Siga_i, q_i = sampler.unpack(g)
    x_full, phi_full = solve_diffusion(
        L, N_CELLS, layer_bounds, D_i, Siga_i, q_i,
        bc_left=('dirichlet', 1.0),
        bc_right=('dirichlet', 1.0),
    )
    x_m, phi_m = mask_solution(x_full, phi_full, PLOT_MIN, PLOT_MAX)
    phi_at_x1.append(phi_m[0])
    phi_at_x5.append(phi_m[-1])
    ax.plot(x_m, phi_m, color=cmap(norm(sigma_a5_values[i])), lw=1.0, alpha=0.8)

print(f"Max flux difference at x≈{PLOT_MIN}: {max(phi_at_x1) - min(phi_at_x1):.4f}")
print(f"Max flux difference at x≈{PLOT_MAX}: {max(phi_at_x5) - min(phi_at_x5):.4f}")

# ------------------------------------------------------------------ #
# Colorbar and labels                                                #
# ------------------------------------------------------------------ #
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
cbar = fig.colorbar(sm, ax=ax)
cbar.set_label(r"$\Sigma_{a,5}$  [cm$^{-1}$]", fontsize=11)

ax.set_xlabel("x  [cm]", fontsize=12)
ax.set_ylabel("φ(x)  [a.u.]", fontsize=12)
ax.set_title(
    f"5-layer slab – varying $\\Sigma_{{a,5}} \\in {XS5_RANGE}$\n"
    f"solution shown on $x \\in [{PLOT_MIN}, {PLOT_MAX}]$",
    fontsize=12,
)
plt.tight_layout()
plt.savefig("output_graphs/varying_constant_xs_layer5.png", dpi=150)
print("Saved: output_graphs/varying_constant_xs_layer5.png")
plt.show()
